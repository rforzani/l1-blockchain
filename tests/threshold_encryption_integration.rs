...
use l1_blockchain::mempool::{ThresholdEngine, ThresholdCiphertext, ThresholdShare};
use l1_blockchain::mempool::encrypted::dkg;
use l1_blockchain::crypto::bls::BlsSigner;
use l1_blockchain::pos::registry::{ValidatorSet, Validator, ValidatorStatus};
use l1_blockchain::state::{Balances, Nonces, Commitments, Available};
use l1_blockchain::fees::FeeState;
use l1_blockchain::types::{Block, BlockHeader, CommitTx, AvailTx, RevealTx, Transaction, AccessList, Hash, Tx, Event};
...
fn test_full_commit_avail_decrypt_flow() {
    ...
    let fee_state = FeeState { commit_base: 100, avail_base: 50, exec_base: 25 };
    ...
    assert_eq!(commit_receipt.outcome, l1_blockchain::types::ExecOutcome::Success);
    ...
    assert_eq!(meta.encrypted_payload.commitment_hash(), encrypted_payload.commitment_hash());
    ...
    for &validator_id in &validator_ids {
        let share = chain.threshold_engine.generate_share(&encrypted_payload, validator_id).unwrap();
        threshold_shares.push(share.clone());
        
        // Add share to chain (simulates receiving shares from validators)
        let can_decrypt = chain.add_threshold_share(commit_tx.commitment, share).unwrap();
        if validator_id == 3 { // After third share
    ...
    assert_eq!(avail_receipt.outcome, l1_blockchain::types::ExecOutcome::Success);
    ...
    let decryption_events: Vec<_> = result.events.iter()
...
fn test_duplicate_share_rejection() {
    ...
    // Add initial share
    let share1 = chain.threshold_engine.generate_share(&encrypted_payload, 1).unwrap();
...
fn test_epoch_mismatch_rejection() {
    ...
    let mut share = chain.threshold_engine.generate_share(&encrypted_payload, 1).unwrap();
...
fn test_share_cleanup() {
    ...
    // Add commitment meta
    commitments.insert(commitment, l1_blockchain::types::CommitmentMeta {
    ...
    // Add share
    let share = chain.threshold_engine.generate_share(&encrypted_payload, 1).unwrap();
...
fn test_byzantine_fault_tolerance_threshold() {
    ...
    let decrypted = chain.threshold_engine.decrypt(&encrypted_payload, shares).unwrap();
